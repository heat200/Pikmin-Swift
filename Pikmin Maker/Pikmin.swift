...
class Pikmin:SKSpriteNode {
    ...
    func setUp() {
        ...
        pikminThrow.positional = true
        pikminSquel.autoplayLooped = false
        pikminSquel.positional = true
    ...
    func carryNutrient(nutrient:Nutrient) {
        ...
        if direction == "Right" {
            walkSpot1 = CGPoint(x: nutrient.position.x - 20, y: nutrient.position.y)
            walkSpot2 = CGPoint(x: locationToTakeTo.x - 20, y: locationToTakeTo.y)
