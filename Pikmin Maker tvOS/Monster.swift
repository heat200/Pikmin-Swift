...
class Monster:SKSpriteNode {
    ...
    var healthBar = SKShapeNode(rect: CGRect(x: 0, y: 30, width: 20, height: 5), cornerRadius: 5)
    
    
    func setUp() {
        healthBar.fillColor = SKColor.greenColor()
        healthBar.strokeColor = SKColor.blackColor()
    ...
    func eatTarget() {
        if life_ticks - last_attack_tick >= 40 {
            ...
            removeAllActions()
            runAction(SKAction.animateWithTextures([SKTexture(imageNamed:"Monster_" + monsterSpecies + "_" + direction + "_Eat"),SKTexture(imageNamed:"Monster_" + monsterSpecies + "_" + direction + "_Stand")], timePerFrame: 0.14, resize: true, restore: false),completion:{
                self.target.kill()
                self.busy = false
            })
        }
